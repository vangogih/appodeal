...
namespace Appodeal.Solitaire.Runtime.Assets
{
    /// <summary>
    ...
    /// <see cref="RuntimeConstants.Assets"/>. I/O service logic only.
    /// </summary>
    internal sealed class SolitaireAssetsLoadUnit : ILoadUnit
    {
        ...
        public async UniTask Load()
        {
            ...
            for (var suit = (Suit)0; (int)suit < RuntimeConstants.Game.Cards.SuitCount; suit++)
            {
                ...
                {
                    string path = string.Format(RuntimeConstants.Assets.Cards.FacePathFormat, suit, rank);
            ...
            _cache.Back = AssetService.R.Load<Sprite>(RuntimeConstants.Assets.Cards.BackPath);
            if (_cache.Back == null)
                Log.Loading.W(LogTag, $"Missing card back sprite at '{RuntimeConstants.Assets.Cards.BackPath}'");

            _cache.CardViewPrefab = LoadPrefab(RuntimeConstants.Assets.Prefabs.CardView);
            _cache.PileViewPrefab = LoadPrefab(RuntimeConstants.Assets.Prefabs.PileView);
            _cache.BoardViewPrefab = LoadPrefab(RuntimeConstants.Assets.Prefabs.BoardView);
        }
